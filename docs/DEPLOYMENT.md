...
Three things in `/api/status` are worth watching:

* `ingest.running` — the poller is alive.
* `ingest.degraded` — **true means the archive is a sample, not a history.**
* `ingest.consecutive_errors` — non-zero means the TDU is unreachable or
  failing; `ingest.last_error` says how.
...
| Symptom | Cause |
|---|---|
| `ingest.degraded` is true | the TDU has no `/spill_history` route; see `contrib/tduweb/` |
| `consecutive_errors` climbing | TDU unreachable; check `ingest.last_error` and `curl http://tdu-.../tcr_running` |
